...
public class TestPiezas extends PartidaAjedrez {
	...
	/**
	 * Este método prueba si se da como correto puedeMover cuando se va a una casilla distinta
	 */
	@Test
	public void testProbarDistintaCasilla() {
		String []piezas = {"ALFIL", "CABALLO", "PEON",
				"REINA","REY","TORRE"};
		int [] incX = {1,1,0,1,1,2};
		int [] incY = {1,-2,1,2,0,0};
		...
		try {
			for (int i =0; i< piezas.length; i++) {
				...
				PiezaAjedrez pieza = (PiezaAjedrez) FactoriaPiezasAjedrez.crearPieza(new ControlJugadoresAjedrez(),
						5, 'd', piezas[i], "BLANCO");
				assertTrue("Es movimiento válido ir a otra casilla distinta que la de partida",
						pieza.puedeMover(pieza.getFilaAjedrez()+incX[i],
								(char)(pieza.getColumnaAjedrez()+incY[i])));	
				pieza = (PiezaAjedrez) FactoriaPiezasAjedrez.crearPieza(new ControlJugadoresAjedrez(),
						5, 'c', piezas[i], "NEGRO");
				assertTrue("Es movimiento válido ir a otra casilla ir a otra casilla distinta que la d",
						pieza.puedeMover(pieza.getFilaAjedrez()+incX[i],
								(char)(pieza.getColumnaAjedrez()+incY[i])));
			}
		} catch (PartidaException e) {
	...
	protected boolean testMovimientos(DatosPrueba datos) {
		...
		if (pieza == null) {
			...
			IList<Casilla> res = pieza.movimientosValidos();
			if (res == null) {
	...
	protected class DatosPrueba {
		...
		private IList<Casilla> casillas;
		 ...
		 */
		public boolean mismasCasillas (IList<Casilla> verificar){//mismasCasillas
