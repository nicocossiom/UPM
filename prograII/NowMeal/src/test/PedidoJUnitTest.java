...
public class PedidoJUnitTest {
	...
	public void testCoste() {
		...
		assertEquals("Coste (" + pedido.coste(moto) + ") se ha calculado incorrectamente", 
				35, pedido.coste(moto), DELTA_PRECISSION);	
		
		PlatoComida comida3 = new PlatoComida("CO1", 50, 1);
		PlatoComida comida4 = new PlatoComida("CO2", 25, 2);
		PlatoComida[] comidasSolicitadas1 = {comida3, comida4};
		
		pedido = new Pedido(cliente, comidasSolicitadas1, restaurante);
		assertEquals("Coste (" + pedido.coste(moto) + ") se ha calculado incorrectamente", 
				107, pedido.coste(moto), DELTA_PRECISSION);
