...
int main(int argc, char *argv[]) {
    ...
    struct sigaction acc = {.sa_handler=SIG_IGN, .sa_flags=0} ;
    sigemptyset(&acc.sa_mask);
    sigaction(SIGCLD, &acc, NULL); // evita que hijos queden zombis sin hacer wait
    while(1) {
        ...
        if (fork()==0) {
            ...
            printf("recibida petición cliente\n");
                ...
                // sleep(5); // para probar que servicio es concurrente
                revierte(dato, tamn);
